"""EU Mean Reversion CAC40 strategy for BacktesterV2.

Hypothesis: The CAC40 exhibits mean-reverting behavior similar to DAX when
price extends significantly from VWAP. However, the French market is slightly
less liquid than Frankfurt, requiring wider parameters (higher ATR multipliers)
to avoid getting stopped out by noise.
...
Exit rules:
  ...
  - Stop loss: price extends further — 3.5x ATR from VWAP (wider than DAX)
  - Time exit: EOD (17:00 CET)

Symbol: CAC40 (Euronext Paris)
Timeframe: 15min or 1H bars
Expected: ~6-10 trades/month, win rate 55-60%, Sharpe 0.8-1.5
"""
...
class EUMeanReversionCAC(StrategyBase):
    """ATR extension mean reversion on CAC40 — fade overextensions from VWAP.

    Wider parameters than DAX variant to account for lower liquidity on
    Euronext Paris, which creates more noise around the mean.
    """

    SYMBOL = "CAC40"

    def __init__(self) -> None:
        # ATR parameters — wider than DAX (French market less liquid)
        self.atr_period: int = 14
        self.atr_entry_mult: float = 2.2
        self.atr_sl_mult: float = 3.5
    ...
    def get_parameter_grid(self) -> Dict[str, List[Any]]:
        return {
            "atr_period": [10, 14, 20],
            "atr_entry_mult": [1.8, 2.2, 2.5, 3.0],
            "atr_sl_mult": [3.0, 3.5, 4.0],
