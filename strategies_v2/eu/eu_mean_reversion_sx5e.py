"""EU Mean Reversion Euro Stoxx 50 strategy for BacktesterV2.

Hypothesis: Euro Stoxx 50 (ESTX50) is the broadest EU benchmark, aggregating
constituents across multiple countries. Overextensions from VWAP are driven by
cross-border flow imbalances that correct as arbitrageurs realign the basket
with its components. The diversified nature of the index makes mean reversion
signals more reliable than single-country indices.
...
# Eurex session in UTC: 08:30-16:00 (= 09:30-17:00 CET)
_SESSION_START_HOUR_UTC = 8
_SESSION_START_MIN_UTC = 30
_SESSION_END_HOUR_UTC = 16

STRATEGY_CONFIG = {
    "name": "eu_mean_reversion_sx5e",
    "id": "EU-MR-SX5E",
...
class EUMeanReversionSX5E(StrategyBase):
    """ATR extension mean reversion on Euro Stoxx 50 — fade overextensions from VWAP.

    The ESTX50 basket has natural mean-reversion properties because
    multi-country arbitrage corrects temporary dislocations faster than
